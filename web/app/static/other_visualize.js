...
const userId = $('.dashboard-page').data('user-id');
...
function renderCalendar(date) {
    ...
    fetch(`/api/other_marked_dates?year=${year}&month=${monthIndex + 1}&user_id=${userId}`)
        .then(res => res.json())
        .then(markedDates => {
...
function fetchAndRender(period) {
    ...
    fetch(`/api/other_workouts?period=${period}&date=${selectedDate}&user_id=${userId}`)
        .then(res => res.json())
        .then(data => {
            // clear old charts
